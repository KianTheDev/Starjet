...
/**
 * Matrix class for RREF calculations.
 ...
 */
{
	...
	public Matrix(double [][] m) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			for (int j = 0; j < numCols; j++) {
				matrix.get(i).add(Fraction.getFraction(m[i][j]));
			}
		}
	}
	...
	public void Scale(Coordinate x, Fraction d) {
		...
		for (int i = 0; i < numCols; i++) {
			row.set(i, row.get(i).multiplyBy(d));
		}
	}
	...
	public void MultiplyAndAdd(Coordinate to, Coordinate from, Fraction scalar) {
		...
		for (int i = 0; i < numCols; i++) {
			row.set(i, row.get(i).add((rowMultiplied.get(i).multiplyBy(scalar))));
		}
	}
		...
		for (int x = 0; x < numCols; x++) {
			...
			//Step 2
				...
				if (getCoordinate(pivot).doubleValue() != 1) {
					...
					*/
					Fraction scalar = getCoordinate(pivot).invert();
					Scale(pivot, scalar);
				}
			//Step 3
				...
				for (int i = pivot.row; i < numRows; i++) {
					...
					Fraction complement = (getCoordinate(belowPivot).negate().divideBy(getCoordinate(pivot)));
				...
				for (int i = pivot.row; i >= 0; i--) {
					if (i == pivot.row) {
						if (getCoordinate(pivot).doubleValue() != 1.0) {
							Scale(pivot, getCoordinate(pivot).invert());	
						}
						continue;
					}
					if (i == pivot.row) {
					...
					Fraction complement = (getCoordinate(abovePivot).negate().divideBy(getCoordinate(pivot)));
